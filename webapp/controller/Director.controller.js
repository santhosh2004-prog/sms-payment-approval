sap.ui.define([
    ...
    return Controller.extend("com.incresol.zpaymentworkflow.controller.Director", {

      
            onInit: function () {
                 var oRouter = this.getOwnerComponent().getRouter();

    oRouter.getRoute("Director").attachPatternMatched(this._validateUserRole, this);
            console.log("Director controller initialized");
...
_loadPaymentData: function () {
    ...
    oModel.read("/PaymentHeaderSet", {
        ...
        success: function (oData) {
            ...
            /* ===================================================== */
            /* 🔹 FILTER ITEMS WHERE AUDITOR STATUS = APPROVED      */
            /* ===================================================== */
            ...
            var aFilteredHeaders = aHeaders.map(function (oHeader) {
                ...
                // 🔹 Keep only items where Auditor Approval Status = APPROVED
                var aApprovedItems = aItems.filter(function (oItem) {
                    ...
                    // 🔹 If last approved stage is Director → Skip
                    // Director index = 4 (PMA=0, HOD=1, CFO=2, AUD=3, DIR=4)
                    if (sLastApprovedStatus >= 4) {
            ...
            console.log("Headers with Auditor Approved items:", aFilteredHeaders.length);
  ...
  handleDialogConfirm: function () {
    ...
    if (sActionType === "REJECT") {
        ...
        aSelectedItems.forEach(function (oItem) {
            ...
            if (!oItem.isHeader) {
                if (!oItem.DirApprRemarks || oItem.DirApprRemarks.trim() === "") {
        ...
        if (aItemsWithoutRemarks.length > 0) {
            console.log("❌ REJECTION BLOCKED - Missing Director remarks");

            var sErrorMessage =
                "Director Remarks are mandatory for rejection.\n\n" +
...
,
        ...
        onPmApprAmountChange: function (oEvent) {
            ...
            // Update the tree data model
            oContext.getModel().setProperty(oContext.getPath() + "/DirApprAmt", sFormattedValue);

            console.log("Director Approved Amount updated in tree data model:", sFormattedValue);
            MessageToast.show("Director Approved Amount updated to ₹" + fNewValue.toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 }));
        },
        ...
        onRemarksChange: function (oEvent) {
            ...
            // Update the tree data model
            oContext.getModel().setProperty(oContext.getPath() + "/DirApprRemarks", sNewValue);
        ...
        ToItems: {
            results: aPayloadItems.map(function (item) {
                return {
                    ...
                    HodApprRemarks: item.HodApprRemarks || "",
                };
            }.bind(this))
        }
    };
